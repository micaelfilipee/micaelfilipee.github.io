LDAvis = function(to_select, json_file) {
	...
	if (xdiff > ydiff) {
        ...
        svg.append("text")
            .text("Mapa da Distância Intertópico (A Partir da Escala Multidimensional)")
        ...
        d3.select("#bar-freqs").append("text")
            ...
            .style("dominant-baseline", "middle")
            .text("Frequência estimada do termo no tópico selecionado");
	...
	// footnotes:
        d3.select("#bar-freqs")
            ...
            .style("dominant-baseline", "middle")
            .text("1. Saliência(term w) = frequency(w) * [sum_t p(t | w) * log(p(t | w)/p(t))] for topics t; see Chuang et. al (2012)");
        d3.select("#bar-freqs")
            .append("a")
            ...
            .style("dominant-baseline", "middle")
            .text("2. Relevância(term w | topic t) = \u03BB * p(w | t) + (1 - \u03BB) * p(w | t)/p(w); see Sievert & Shirley (2014)");
	...
	// dynamically create the topic and lambda input forms at the top of the page:
	    ...
	    clear.innerHTML = "Limpar Tópicos";
		...
		.text("Deslize para ajustar a métrica de relevância:");
		...
		.text("Top-" + R + " Termos mais salientes");
	    title.append("tspan")
	     	.attr("baseline-shift", "super")	    
	     	.attr("font-size", 12)
	     	.text(1);
